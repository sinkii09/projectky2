...
public class ServerGameManager : IDisposable
{
    ...
    public ServerGameManager(string serverIP, int serverPort, int serverQPort, NetworkManager manager)
    {
        ...
        m_NetworkServer = new NetworkServer(manager);
    ...
    public async Task StartGameServerAsync(GameInfo gameInfo)
    {
        Debug.Log($"Starting server with:{gameInfo}.");
        ...
        try
        {
            ...
            if (matchmakerPayload != null)
            {
                ...
                gameInfo = PickGameInfo(matchmakerPayload);

                MatchStartedServerQuery(gameInfo);
                await StartBackfill(matchmakerPayload, gameInfo);
        ...
        if (!m_NetworkServer.OpenConnection(m_ServerIP, m_ServerPort, gameInfo))
        {
            Debug.LogError("NetworkServer did not start as expected.");
            return;
        }
        m_SynchedServerData = await m_NetworkServer.ConfigureServer(gameInfo);
